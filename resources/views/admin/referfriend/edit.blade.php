...
<div class="container-fuild ml-15" >
    ...
    @if(isset($data->id))
        <form class="form-material" action="{{ route('referfriend.update',$data->id) }}" method="post">
    @else
    ...
    <div class="form-group">
      ...
      <input type="text" class="form-control " id="title" autocomplete="off"  placeholder="Enter Title" name="text" value="@if(isset($data->text)){{ $data->text }}@else{{ old('text') }}@endif">
    @if($errors->first('text'))
    <span class="text text-danger">* {{ $errors->first('text') }}</span>
    @endif
    </div>  
    <button type="submit" class="btn btn-default">Submit</button>
    <a href="{{ route('referfriend.sales.button',$data->id) }}" class="btn btn-default">Cancel</a>
  </form>
</div>
    </div>
</div>
