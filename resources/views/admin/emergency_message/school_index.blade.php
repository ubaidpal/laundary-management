...
<div class="container-fuild">
    ...
    <h2 class="pull-left">Emergency messages</h2>
      @if(\Auth::user()->type == 'ADMIN')
    <a href="{{ route('emergencymessage.schoolform')}}" class="btn btn-primary pull-right mt-20">School</a>
    @else
    <a href="{{ route('staff.emergencymessage.schoolform')}}" class="btn btn-primary pull-right mt-20">School</a>
    @endif
  </div>
  ...
  <table class="table" id=myTable>
    ...
    <tbody>
       @foreach($data as $key=> $detail)
            <tr>
              <td>{{ $key+1 }}</td>
              <td>{{ $detail->schools->school_name }}</td>
              <td>{{ $detail->date }}</td>
              <td>{{ $detail->time }}</td>
              <td>{{ $detail->message }}</td>
              @if(\Auth::user()->type == 'ADMIN')
              <td> 
                <a href="{{ route('emergencymessage.showSchoolEditform', $detail->id) }}"> <i class="mdi mdi-account-edit text-default"></i> </a>&nbsp;&nbsp;&nbsp;
                
                <a class="delete deleteuserbutton" href="javascript:void(0)" data-userid="{{$detail->id}}" data-url="{{ route('emergencymessage.schooldeteled') }}"><i class="icon-trash text-default"></i> </a></td>
                @else
                <td> 
                <a href="{{ route('staff.emergencymessage.showSchoolEditform', $detail->id) }}"> <i class="mdi mdi-account-edit text-default"></i> </a>&nbsp;&nbsp;&nbsp;
                
                <a class="delete deleteuserbutton" href="javascript:void(0)" data-userid="{{$detail->id}}" data-url="{{ route('staff.emergencymessage.schooldeteled') }}"><i class="icon-trash text-default"></i> </a></td>
                @endif
            </tr>
          @endforeach 
    </tbody>
