...
<div class="container-fuild">
  ...
  @if(\Auth::user()->type == 'ADMIN')
    <a href="{{ route('emergencymessage.usersadd')}}" class="btn btn-primary pull-right mt-20">Users</a>
    @else
    <a href="{{ route('staff.emergencymessage.usersadd')}}" class="btn btn-primary pull-right mt-20">Users</a>
    @endif
  ...
  <table class="table" id=myTable>
    ...
    <tbody>
          ...
          @foreach($data as $key=> $detail)
            <tr>
              <td>{{ $key+1 }}</td>
              <td>{{ $detail->users->first_name.' '.$detail->users->last_name }}</td>
              <td>{{ $detail->date }}</td>
              <td>{{ $detail->time }}</td>
              <td>{{ $detail->message }}</td>
              @if(\Auth::user()->type == 'ADMIN')
              <td>
                <a href="{{ route('emergencymessage.usersupdate', $detail->id) }}"> <i class="mdi mdi-account-edit text-default"></i> </a>&nbsp;&nbsp;&nbsp;
                <a class="delete deleteuserbutton" href="javascript:void(0)" data-userid="{{$detail->id}}" data-url="{{ route('emergencymessage.usersdelete') }}"><i class="icon-trash text-default"></i> </a></td>
                @else <td>
                <a href="{{ route('staff.emergencymessage.usersupdate', $detail->id) }}"> <i class="mdi mdi-account-edit text-default"></i> </a>&nbsp;&nbsp;&nbsp;
                <a class="delete deleteuserbutton" href="javascript:void(0)" data-userid="{{$detail->id}}" data-url="{{ route('staff.emergencymessage.usersdelete') }}"><i class="icon-trash text-default"></i> </a></td>
                @endif
            </tr>
          @endforeach 
    </tbody>
