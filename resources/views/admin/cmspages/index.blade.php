...
<div class="container-fuild">
    ...
    <h2 class="pull-left">Manage CMS Pages</h2>
  <a href="{{route('cmspages.single')}}" class="btn btn-primary pull-right mt-20">Add   Page</a>
  </div>
  ...
  <table class="table" id=myTable>
    ...
    <tbody>
      ...
      @foreach ($data as $details)
        <tr>
        <td>{{$j++}}</td>
        <td>{{URL::to('/').'/'.$details->url }} </td>
        <td>{{$details->title }} </td>
        <td>@if(strlen(strip_tags(str_replace(array_keys($replace), array_values($replace),$details->description))) >= 20 ){{  substr(strip_tags(str_replace(array_keys($replace), array_values($replace),$details->description)),0,20).'....' }}@else{{ strip_tags(str_replace(array_keys($replace), array_values($replace),$details->description)) }}@endif</td>
        <td>
            <a href="{{ route('cmspages.editForm', $details->id) }}"> <i class="mdi mdi-account-edit   text-default"></i> </a>&nbsp;&nbsp;&nbsp;
            <a  class="delete deleteuserbutton" href="javascript:void(0)" data-userid="{{$details->id}}" data-url="{{ route('cmspages.delete') }}" ><i class="icon-trash text-default"></i> </a>
        </td>
        </tr>
    @endforeach
    </tbody>
        ...
        $.ajax({
          ...
          success:function(data){
            ...
            if(data.error == false){
              swal({ 
                  button: false,
                  text: "Sorry you can`t delete this page!"
              });
