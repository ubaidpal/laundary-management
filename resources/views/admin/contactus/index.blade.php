...
<div class="container-fuild">
    <div class="card">
      <div class="card-body">
        <div>
        <h2 class="pull-left">Manage Contact us</h2>
          <!-- <a href="{{ route('fees.single') }}" class="btn btn-primary pull-right mt-20">Add Fees</a> -->
        </div>
  ...
  <table class="table" id="myTable">
    ...
    <tbody>
      ...
      @foreach ($data as $details)
        ...
        <td>{{$details->email}}</td>
        <td>@if(!empty($details->schoolname->school_name)){{ $details->schoolname->school_name }} @endif</td>
        <td>{{$details->country_code.' '.$details->contact}}</td>
        <td class="show-read-more">{{$details->message}}</td>
        <td>
            <a class="delete deleteuserbutton" href="javascript:void(0)" data-userid="{{$details->id}}" data-url="{{ route('contactus.delete') }}" ><i class="icon-trash text-default"></i> </a>
        </td>
      </tr>
    @endforeach 
    </tbody>
        ...
        $.ajax({
          ...
          success:function(data){
            ...
            if (data.status == false) { 
              $('#delete_password_error').text(data.success);
