...
<div class="container-fuild">
    ...
    <h2 class="pull-left">Manage FAQ's</h2>
  <a href="{{route('faqs.single')}}" class="btn btn-primary pull-right mt-20">Add Question</a>
  </div>
  ...
  <table class="table" id="myTable">
    ...
    <tbody>
      ...
      @foreach ($data as $details)
        ...
        <td>@if(strlen(strip_tags(str_replace(array_keys($replace), array_values($replace),$details->answer))) >= 75 ){{  substr(strip_tags(str_replace(array_keys($replace), array_values($replace),$details->answer)),0,50).'....' }}@else{{ strip_tags(str_replace(array_keys($replace), array_values($replace),$details->answer )) }}@endif</td>
        <td>
            <a href="{{ route('faqs.editForm', $details->id) }}"> <i class="mdi mdi-account-edit text-default"></i> </a>&nbsp;&nbsp;&nbsp;
            <a class="delete deleteuserbutton" href="javascript:void(0)" data-userid="{{$details->id}}" data-url="{{ route('faqs.delete') }}"><i class="icon-trash text-default"></i> </a>
        </td>
        </tr>
    @endforeach
    </tbody>
