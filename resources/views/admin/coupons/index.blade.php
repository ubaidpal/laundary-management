...
<div class="container-fuild">
    ...
    <div class="card-body">
    <div>
  <h2 class="pull-left">Manage Coupons</h2>
    <a href="{{ route('coupons.single') }}" class="btn btn-primary pull-right mt-20">Add Coupon</a>
    </div>
  ...
  <table class="table" id="myTable">
    ...
    <tbody>
      ...
      @foreach ($data as $details)
        ...
        <td>{{$j++}}</td>
        <td>
          @if(!empty($details->upload_icon))
          <img id="blah" src="{{ $details->upload_icon }}" alt="your image" width="50" height="50">
        @else
          <img id="blah" src="{{asset('images/preview-icon.png')}}" alt="your image" width="50" height="50">
        @endif
        ...
        <td>{{$details->code}}</td>
        <td>@if($details->coupon_type == '1') <span class="label label-success"> %</span> @else<span class="label label-primary">$</span>@endif</td>
        {{--<td>
            @if(isset($details->total) && !empty($details->total))
                ${{$details->total}}
            @else

            @endif
        </td>--}}
       <td>@if($details->status == 1)<a  href="javascript:void(0)" data-userid="{{$details->id}}" data-status="0" data-url="{{ route('coupons.status') }}" class="text-success active_account">{{ 'Activated' }}</a> @else <a href="javascript:void(0)" data-userid="{{$details->id}}" data-status="1" data-url="{{ route('coupons.status') }}" class="text-danger inactive_account"  > {{ 'Inactived' }} </a> @endif</td>
        <td>
            <a href="{{ route('coupons.showupdate', $details->id) }}"> <i class="mdi mdi-account-edit text-default"></i> </a>&nbsp;&nbsp;&nbsp;
            <a class="delete deleteuserbutton" href="javascript:void(0)" data-userid="{{$details->id}}" data-url="{{ route('coupons.delete') }}"><i class="icon-trash text-default"></i> </a>
        </td>
      </tr>
    @endforeach
    </tbody>
    ...
    <form class="modal-content" action="{{ route('users.status') }}" id="contactForm" method="post">
      ...
      <div class="container">
       <!--  <h1>Account inactive</h1> -->
        <p>Are you sure you want to inactivate this coupon?</p> 
...
<!-- Modal --> 
 ...
 -->    <form class="modal-content" action="{{ route('users.status') }}" id="contactForm2" method="post">
      ...
      <div class="container">
        ...
        <p>Are you sure you want to active this coupon?</p> 
        <div class="form-group">
          <label for="name">Password:</label>
          ...
          success:function(data){ 
            ...
            if (data.status == false) {
              ...
              swal({
                  ...
                  button: false,
                  text: "Coupon inactive successfully"
              }); 
        ...
        $.ajax({
          ...
          success:function(data){
            ...
            if (data.status == false) { 
              ...
              swal({ 
                  button: false,
                  text: "Coupon active successfully"
              }); 
              window.setTimeout(function(){ 
                  location.reload();
              } ,1500);
