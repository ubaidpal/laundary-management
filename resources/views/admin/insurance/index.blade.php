...
<div class="container-fuild">
    ...
    <h2 class="pull-left">Manage Insurance Table</h2>
    <a href="{{ route('insurance.single') }}" class="btn btn-primary pull-right mt-20">Add Plan</a>
  ...
  <table class="table" id=myTable>
    ...
    <tbody>
      ...
      @foreach ($data as $details)
        ...
        <?php
          $pricess = explode(',', $details->prices);

        ?>
        @foreach($pricess as $k=>$val)
            @if($val == '-')
            <td>{{ $val }}</td>
            @else
            <td>${{ $val }}</td>
        ...
        @endforeach 
          <td>
            <a href="{{ route('insurance.editForm', $details->id) }}"> <i class="mdi mdi-account-edit   text-default"></i> </a>&nbsp;&nbsp;&nbsp;
            <a class="delete deleteuserbutton" href="javascript:void(0)" data-userid="{{$details->id}}" data-url="{{ route('insurance.delete') }}"><i class="icon-trash text-default"></i> </a>
        </td>
        </tr>
    @endforeach
        ...
        $.ajax({
          ...
          success:function(data){
            ...
            if (data.status == false) { 
              ...
              swal({ 
                  button: false,
                  text: "Insurance plan deleted successfully"
              }); 
