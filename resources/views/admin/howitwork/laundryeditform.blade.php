...
<div class="container-fuild ml-15" >
    ...
    <div class="card-body">
        ...
        <h2>How it work</h2>
        <form class="form-material" action="{{ route('how_it_work.update',['id' => $data->id,'type'=>$type]) }}" method="post">
    
            {!! csrf_field() !!} 
            
           
            <input type="hidden" name="work_detail_id" value="{{ $data->id }}">
            ...
            <textarea  class="form-control" required="" placeholder="Enter description" name="description">{{ $data->description }}</textarea>
            @if($errors->first('description'))
            <span class="text text-danger">* {{ $errors->first('description') }}</span>
            @endif
             
            <button type="submit" class="btn btn-default">Submit</button>
            <a href="{{ route('how_it_work.index',['type' =>$type]) }}" class="btn btn-default">Cancel</a>
  </form>
</div>
    </div>
</div>
