...
<div class="container-fuild">
  ...
  <div class="pull-right w35">
    @if(\Auth::user()->type == 'ADMIN')
  <a href="{{ route('users.single') }}" class="btn btn-primary pull-right">Add New Customer</a>
    @else
  <a href="{{ route('staff.users.single') }}" class="btn btn-primary pull-right">Add New Customer</a>
    @endif
  </div>
  <br>
  ...
  @endif
  <div class="hideClass" id="test" style="padding:5px;width:50%">
  </div>
    ...
    <tbody>
      ...
      @foreach ($data as $details)
        ...
        <td>{{$details->school_name}}</td>
         ...
         @if(\Auth::user()->type == 'ADMIN')
        <td>
          @if($details->status == 1)<a href="javascript:void(0)" data-userid="{{$details->id}}" data-status="0" data-url="{{ route('users.status') }}" class="text-success active_account">{{ 'Activated' }}</a> @else <a href="javascript:void(0)" data-userid="{{$details->id}}" data-status="1" data-url="{{ route('users.status') }}" class="text-danger inactive_account"> {{ 'Inactived' }} </a> @endif
        </td>
        <td>
            <a href="{{ route('users.view', $details->id) }}"> <i class="mdi mdi-eye text-default"></i> </a>&nbsp;
            <a href="{{ route('users.edit', $details->id) }}"> <i class="mdi mdi-account-edit text-default"></i> </a>&nbsp;
            <a class="delete deleteuserbutton" href="javascript:void(0)" data-userid="{{$details->id}}" data-url="{{ route('users.delete') }}"><i class="icon-trash text-default"></i> </a>
        </td>
        @else
        <td>
          @if($details->status == 1)<a href="javascript:void(0)" data-userid="{{$details->id}}" data-status="0" data-url="{{ route('staff.users.status') }}" class="text-success active_account">{{ 'Activated' }}</a> @else <a href="javascript:void(0)" data-userid="{{$details->id}}" data-status="1" data-url="{{ route('staff.users.status') }}" class="text-danger inactive_account"> {{ 'Inactived' }} </a> @endif
        </td>
        <td>
            <a href="{{ route('staff.users.view', $details->id) }}"> <i class="mdi mdi-eye text-default"></i> </a>&nbsp;
            <a href="{{ route('staff.users.edit', $details->id) }}"> <i class="mdi mdi-account-edit text-default"></i> </a>&nbsp;
            <a class="delete deleteuserbutton" href="javascript:void(0)" data-userid="{{$details->id}}" data-url="{{ route('staff.users.delete') }}"><i class="icon-trash text-default"></i> </a>
        </td>
        @endif
      </tr>
    @endforeach
  ...
  <form class="modal-content" action="{{ route('users.status') }}" id="contactForm" method="post">
      ...
      <div class="container">
       <!--  <h1>Account inactive</h1> -->
        ...
        <button type="button" class="cancelbtn cancelbtn3" data-dismiss="modal">Cancel</button>
...
<script>
  ...
  /*$('.active_account').click(function(){
      var r = confirm("Are you sure you want to inactive this account!");
      if (r == true) {
          return true;
      } else {
          return false;
